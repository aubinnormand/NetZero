...
def register_callbacks(app, df_data, gdf_world, norm_map):
    ...
    def update_map(indicator, database, type_value, year, scale, color_range, normalization):
        ...
        if no_data_at_all:
            ...
            tickvals, ticktext = [], []
            if scale == 'absolute':
                pos_ticks = np.linspace(0, zmax, 4 + 1)[1:]
                neg_ticks = np.linspace(zmin, 0, 4 + 1)[:-1]
                tickvals = np.concatenate([neg_ticks, [0], pos_ticks])
            elif scale in ['relative', 'rank', 'log']:
